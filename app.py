    ...
    try:
        ...
        if compare_all:
            ...
            if results:
                ...
                # --- Forecast Visualization with Date Axis ---
                best_forecast = all_forecasts[best_model]
                last_date = df.index[-1]
                freq = pd.infer_freq(df.index) or "D"
                forecast_dates = pd.date_range(start=last_date, periods=forecast_horizon + 1, freq=freq)[1:]

                fig = go.Figure()
                fig.add_trace(go.Scatter(x=df.index, y=df[target_column].values, name="Actual", line=dict(color="blue")))
                fig.add_trace(go.Scatter(
                    ...
                    mode="lines+markers",
                    text=[f"{d.date()}: {v:.2f}" for d, v in zip(forecast_dates, best_forecast.values)],
                    hoverinfo="text"
                ))
                fig.add_vline(x=last_date, line=dict(color="gray", dash="dot"),
                              annotation_text="Forecast Start", annotation_position="top left")
                fig.update_layout(xaxis_title="Date", yaxis_title=target_column, hovermode="x unified")
                st.plotly_chart(fig, use_container_width=True)

                st.subheader("📅 Forecasted Values (Best Model)")
                forecast_df = pd.DataFrame({"Date": forecast_dates, "Forecast": best_forecast.values})
        ...
        # --- Single Model Forecast ---
            ...
            fig.add_trace(go.Scatter(
                ...
                mode="lines+markers",
                text=[f"{d.date()}: {v:.2f}" for d, v in zip(forecast_dates, forecast.values)],
                hoverinfo="text"
            ))
            fig.add_vline(x=last_date, line=dict(color="gray", dash="dot"),
                          annotation_text="Forecast Start", annotation_position="top left")
